...
class ChannelParallelConvolution3D(chainer.links.Convolution3D):
    def __init__(self, comm, in_channels, out_channels, *args, **kwargs):
        ...
        if self.comm.size > self.in_channels:
            ...
            super(ChannelParallelConvolution3D, self).__init__(self.new_in_channels, self.out_channels, *args, **kwargs)

    def __call__(self, x, *args, **kwargs):
        # Each process gets C/P channels
        x = x[:, self._channel_indices, :, :]
        y = super(ChannelParallelConvolution3D, self).__call__(x)
