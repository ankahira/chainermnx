...
class FilterNcclCommunicator(mpi_communicator_base.MpiCommunicatorBase):
    ...
    def nccl_allgather(self, x, comm):
        ...
        if comm.rank == 0:
            print("{:.10f}".format(stop-start), "\t", "{:.10f}".format(cpu_stop-cpu_start), file=self.allgather_time_file)
    ...
    def nccl_allreduce(self, input, comm):
        ...
        if comm.rank == 0:
            print("{:.10f}".format(stop-start), "\t", "{:.10f}".format(cpu_stop-cpu_start), file=self.allreduce_time_file)
