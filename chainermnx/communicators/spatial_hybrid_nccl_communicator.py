...
class SpatialHybridNcclCommunicator(mpi_communicator_base.MpiCommunicatorBase):
    ...
    def finalize(self):
        super(SpatialHybridNcclCommunicator, self).finalize()
    ...
    def set_config(self, name, value=True, **kwargs):
        if name == 'allreduce_grad_dtype':
            ...
            super(SpatialHybridNcclCommunicator, self).set_config(name, **kwargs)

    def get_config(self, name=None):
        if name == 'allreduce_grad_dtype':
            return self.allreduce_grad_dtype
        else:
            return super(SpatialHybridNcclCommunicator, self).get_config(name)
    ...
    def _intra_node_mean_grad_async(self, model, zero_fill, stream):
        ...
        # Allreduce from buffer A -> buffer B
        # div by comm_size from buffer B -> buffer A
        self._intra_node_mean_nccl(self.gpu_buffer_a, self.gpu_buffer_b,
                                   n_elems,
                                   allreduce_grad_dtype, stream)
    ...
    def _intra_node_mean_nccl(self, sendbuf, recvbuf,
                              n_elems, dtype, stream=None):
        """

        This function is the modified version of _multi_node_mean_nccl.
        Instead of dividing by number of process. We dont take the mean
        This is to facilate spatial parallelism where mean is not required
        for processess in the same node
