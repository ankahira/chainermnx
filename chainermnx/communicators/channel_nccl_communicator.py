...
class ChannelNcclCommunicator(channel_mpi_communicator_base.MpiCommunicatorBase):
    ...
    def finalize(self):
        super(ChannelNcclCommunicator, self).finalize()
    ...
    def set_config(self, name, value=True, **kwargs):
        if name == 'allreduce_grad_dtype':
            ...
            super(ChannelNcclCommunicator, self).set_config(name, **kwargs)

    def get_config(self, name=None):
        if name == 'allreduce_grad_dtype':
            return self.allreduce_grad_dtype
        else:
            return super(ChannelNcclCommunicator, self).get_config(name)
